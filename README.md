...
├── 06-hub/           HU：门面站前端（index/join/points/proposals/worldview.html + assets css/js）
...
| 文档 | 说明 |
|---|---|
| [`02-co-creation/creator-management-system.md`](02-co-creation/creator-management-system.md) | **运营体系主档**（OWKV-CREATOR-SYS v2.2）：一个 Hub + 平台矩阵入口、同一事实源、决策表（D1-D7）、共创流水线、积分看板、公示四档；D2 报名站内注册表单（CF Worker→D1，主）自动建档观察者 + GitHub（熟手）/Discord（补充）辅助，创作者路A积分转正/路B直接申请，仅人工引导展位不入数据流 |
| [`02-co-creation/contribution-data-spec.md`](02-co-creation/contribution-data-spec.md) | **贡献力数据规范**（OWKV-DATA-SPEC v1）：注入源站内表单(CF·主)/邮件/GitHub/Discord（不入流）、C1-C4 单次计分规则、表结构契约（events_raw/contributors/ledger/tasks/v_dashboard）、快照 schema、容量演进；落地见 cf-owkv-deploy（CF D1 + Worker 已部署） |
...
> 共用一套 URL，只做展示与入口，不建第二套系统、不建自建 DB。讨论区归平台矩阵（Discord / 群 / Reddit）。正典库本体就在本仓库（00-core 等）。
...
- ⚠️ 发布源注意：GtHub Pages 从仓库根渲染 HTML，HU现已在 `06-hub/` 子目录——若 Pages 配置为根渲染需调整发布目录为 `06-hub/`，或保持同根并调整（详见 PUBLIC-LAUNCH 与 hub 规划）
