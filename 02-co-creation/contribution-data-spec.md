...
## 1. 信息注入源（已修订，符合 D2：站内注册表单为主入口）

| 入口 | 自动/半自动 | 注入方式 | 事件类型示例 |
|---|---|---|---|
| **站内注册表单**（CF Worker→D1，主入口） | **自动** | openwkv.xyz/join.html 原生表单（匿名代号/角色/模态/邮箱）→ CF Worker 校验(Turnstile+蜜罐+域名黑名单) → 直写 `events_raw`；邮箱只存哈希 | join |
...
| **微信** | **已移除** | 已退出所有沟通职能、不列入平台矩阵（2026-08-29 v2.3 定） | — |
| **小红书** | **不注入** | 仅作国内画师展示/触达，**不入数据流** | —（展示/视觉） |
...
- **建库与注入脚本已落地**：CF D1 `owkv-events` 已建（schema.sql），报名表单 Worker `owkv-enroll` 已部署（worker.js，含自动建表 / Turnstile+蜜罐+域名黑名单 / 直写 `events_raw`）。表结构以此处契约 + schema.sql 双向一致为准。
- **ETL 计分 / 导出快照 / 公示喂数**：属私有中枢实现阶段（DB 已就绪，待导出链路接入）。
- **不碰分润结算**（D4 挂起，链条到「公示」为止）。
- **服务器 202.61.255.243**：仅作可选回流镜像，非主库（不擅动，待用户给 SSH 接入方式）。
